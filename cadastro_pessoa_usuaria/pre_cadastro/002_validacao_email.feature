Feature: Validação do e-mail
    ...
    Scenario: Verificar mensagem informando o tempo que o link de validação ficará ativo
    Then o sistema deve exibir uma mensagem informando quanto tempo o link de validação
    ficará ativo

    Scenario: Verificar redirecionamento para a tela de Login ao selecionar botão
    When paciente selecionar o botão "Voltar ao Login"
    Then deve ser redirecionado para a tela de Login

    Scenario: Validar link para reenviar solicitação de validação do e-mail
